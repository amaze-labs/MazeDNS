...
- **Join token mismatch or missing.** The agent's `MAZEDNS_JOIN_TOKEN` must be a
  ...
  credential (join token or node key) are set. If neither is set the agent runs
  standalone and never appears in the cluster.
