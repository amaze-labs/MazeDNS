...
func TestAgentSync(t *testing.T) {
	...
	reloaded := false
	ag := NewAgent(ts.URL, "tok", time.Second, st, func() error { reloaded = true; return nil }, func() store.NodeStats { return store.NodeStats{} })
	...
	bad := NewAgent(ts.URL, "wrong", time.Second, st, nil, nil)
